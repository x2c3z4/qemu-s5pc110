...
static uint32_t pl192_priority_sorter(pl192_state *s)
{
    ...
    for (i = 0; i < PL192_PRIO_LEVELS; i++) {
        if ((s->sw_priority_mask & (1 << i)) && prio_irq[i] <= PL192_DAISY_IRQ) {
            return prio_irq[i];
        }
    }
    return PL192_NO_IRQ;
...
static void pl192_reset(pl192_state *s)
{
    int i;
...
static int pl192_init1(SysBusDevice *dev)
{
    ...
    iomemtype = cpu_register_io_memory(pl192_readfn, pl192_writefn, s);
...
{
    sysbus_register_dev("pl192", sizeof(pl192_state), pl192_init1);
}
